  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex justify-between items-center">
        ...
        <button onClick={loadRequests} className="text-indigo-600 hover:text-indigo-800 font-medium text-sm flex items-center bg-indigo-50 px-3 py-1.5 rounded-lg transition-colors">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {requests.map(req => (
            <div key={req.id} className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow">
                ...
                <div className="text-sm text-gray-500 mb-4 flex justify-between">
                    ...
                    <span className="font-bold text-indigo-600 text-lg">Qty: {req.quantity}</span>
                </div>
                ...
                <div className="flex gap-3">
                    ...
                    <button onClick={() => requestAction(req.id, 'approve')} className="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all font-semibold text-sm shadow-sm shadow-indigo-600/20 active:scale-95">
                        Approve
                    </button>
                </div>
            </div>
        ...
        {requests.length === 0 && (
            <div className="col-span-full bg-white rounded-2xl shadow-sm border border-gray-100 p-16 text-center">
                ...
                <p className="text-gray-500 text-sm">All workshop requests have been processed.</p>
            </div>
        )}
      </div>
      ...
      {confirmModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center animate-in zoom-in-95 duration-200">
              <div className={`mx-auto flex items-center justify-center h-14 w-14 rounded-full mb-4 ${confirmModal.isDestructive ? 'bg-red-100 text-red-600' : 'bg-indigo-100 text-indigo-600'}`}>
              ...
              <div className="flex justify-center gap-3">
                ...
                <button onClick={confirmAction} className={`flex-1 px-4 py-2.5 font-semibold text-white rounded-xl transition-colors shadow-sm ${confirmModal.isDestructive ? 'bg-red-600 hover:bg-red-700 shadow-red-600/20' : 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-600/20'}`}>
